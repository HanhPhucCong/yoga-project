...
// Kết nối đến MongoDB
mongoose.connect('mongodb://localhost:27017/courses-yoga', {
...
const seedData = async () => {
  try {
    ...
    const courses = await Course.insertMany([
      { title: 'JavaScript Basics', description: 'Learn the basics of JavaScript', price: 100, category: categories[0]._id, status: 'published' },
      { title: 'Advanced CSS', description: 'Master CSS for modern web design', price: 150, category: categories[1]._id, status: 'draft' },
      { title: 'Digital Marketing 101', description: 'An introduction to digital marketing', price: 120, category: categories[2]._id, status: 'published' },
    ]);
    ...
    const lectures = await Lecture.insertMany([
      { title: 'What is JavaScript?', contentUrl: 'http://example.com/js_intro.mp4', sectionId: sections[0]._id, contentType: 'video', duration: 10 },
      { title: 'Variables and Data Types', contentUrl: 'http://example.com/js_variables.mp4', sectionId: sections[0]._id, contentType: 'video', duration: 15 },
    ]);
